...
case "$1" in
  local)
    ...
    echo "Database connected! Flushing expired tokens..."
    docker compose -f docker-compose.yml exec -T backend python manage.py flushexpiredtokens --traceback
  ...
  prod)
    ...
    echo "Database connected! Flushing expired tokens..."
    docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T backend python manage.py flushexpiredtokens
  ...
  nuke-tokens)
    echo "Forcefully deleting ALL tokens from the database..."
    # The -c flag lets you pass a Python string directly into the Django shell without opening the interactive prompt
    docker compose -f docker-compose.yml exec backend python manage.py shell -c "from rest_framework_simplejwt.token_blacklist.models import OutstandingToken; OutstandingToken.objects.all().delete()"
    echo "All tokens destroyed."
    ;;
